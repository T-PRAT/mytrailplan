import type { Section, ProfilePoint, Segment, GapSectionResult, GapSimulationResult } from '../types';
...
export function minettiCost(i: number): number {
  return 155.4 * i ** 5 - 30.4 * i ** 4 - 43.3 * i ** 3 + 46.3 * i ** 2 + 19.5 * i + CR_FLAT;
}
...
export function simulateGap(sections: Section[], gapPaceSecondsPerKm: number): GapSimulationResult {
  ...
  const averageActualPace = totalDist > 0 ? totalTime / (totalDist / 1000) : gapPaceSecondsPerKm;

  return { sections: gapSections, totalTime, averageActualPace, gapPace: gapPaceSecondsPerKm };
}
 ...
 */
export function detectSegments(sections: Section[], profilePoints: ProfilePoint[]): Segment[] {
  ...
  for (let i = 1; i < profilePoints.length; i++) {
    ...
    if (direction === null) {
      ...
      if (highPoint - ele > REVERSAL_TOLERANCE) {
        ...
        const eleChange = profilePoints[highIdx].elevation - profilePoints[segStartIdx].elevation;
        if (Math.abs(eleChange) >= MIN_ELEVATION_CHANGE) {
          pushSegment(segments, sections, profilePoints, 'climb', segStartIdx, highIdx);
        }
      ...
      if (ele - lowPoint > REVERSAL_TOLERANCE) {
        const eleChange = profilePoints[lowIdx].elevation - profilePoints[segStartIdx].elevation;
        if (Math.abs(eleChange) >= MIN_ELEVATION_CHANGE) {
          pushSegment(segments, sections, profilePoints, 'descent', segStartIdx, lowIdx);
        }
  ...
  if (direction !== null) {
    ...
    const eleChange = profilePoints[endIdx].elevation - profilePoints[segStartIdx].elevation;
    if (Math.abs(eleChange) >= MIN_ELEVATION_CHANGE) {
      pushSegment(
        segments, sections, profilePoints,
...
function pushSegment(
  ...
  if (endSectionIdx < startSectionIdx) return;
  ...
  segments.push({
    ...
    distance,
    elevationChange: profilePoints[endPtIdx].elevation - profilePoints[startPtIdx].elevation,
...
function fillGaps(
  ...
  if (sections.length === 0) return significantSegments;

  const lastSectionIdx = sections.length - 1;
  const result: Segment[] = [];

  // Trier les segments significatifs par position
  const sorted = [...significantSegments].sort((a, b) => a.startIndex - b.startIndex);
  ...
  for (const seg of sorted) {
    ...
    if (seg.startIndex > cursor) {
      result.push(makeGapSegment(sections, profilePoints, cursor, seg.startIndex - 1));
    }
    result.push(seg);
  ...
  if (cursor <= lastSectionIdx) {
    result.push(makeGapSegment(sections, profilePoints, cursor, lastSectionIdx));
  }
...
function makeGapSegment(
  ...
  const endPt = profilePoints[endIdx + 1] ?? profilePoints[profilePoints.length - 1];
 ...
 */
export function gapPaceFromTime(sections: Section[], totalSeconds: number): number {
  const weightedDist = sections.reduce((acc, s) => acc + s.distance * costRatio(s.slope), 0);
  if (weightedDist === 0) return 360;
...
export function parseDuration(input: string): number | null {
  ...
  // "4h30m", "4h30", "4h"
  const hm = s.match(/^(\d+)h(\d+)m?$|^(\d+)h$/);
  if (hm) {
    const h = parseInt(hm[1] ?? hm[3], 10);
  ...
  if (mins) {
    const m = parseInt(mins[1], 10);
    if (m <= 0 || m > 5000) return null;
    return m * 60;
  }
