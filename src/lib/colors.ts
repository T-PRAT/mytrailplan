...
function parseHex(hex: string): [number, number, number] {
  ...
  return [Math.floor(n / 65_536) % 256, Math.floor(n / 256) % 256, n % 256];
}
...
export function interpolateHexColors(
  count: number,
  palette: "uphill" | "downhill"
): string[] {
  const anchors = (palette === "uphill" ? UPHILL_COLORS : DOWNHILL_COLORS).map(
    (c) => parseHex(c.hex)
  );
  if (count === 1) {
    return [toHex(...anchors[0])];
  }
  return Array.from({ length: count }, (_, i) => {
    ...
    const frac = pos - lo;
    const [r, g, b] = anchors[lo].map(
      (c, j) => c + frac * (anchors[hi][j] - c)
