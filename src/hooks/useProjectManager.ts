...
import {
  ...
  renameProject as persistRenameProject, createProject as persistCreateProject,
  getActiveProjectId, setActiveProjectId, migrateLocalStorageFoodLibrary,
} from '../lib/persistence';
import type { ProjectMeta, StoredProject, NutritionState } from '../types';
...
export function useProjectManager() {
  ...
  useEffect(() => {
    Promise.all([listProjects(), getActiveProjectId()]).then(async ([list, activeId]) => {
      setProjects(list);
  ...
  const createProject = useCallback(async (gpxText: string, filename: string) => {
    if (debounceRef.current) { clearTimeout(debounceRef.current); debounceRef.current = null; }
    const migratedLibrary = migrateLocalStorageFoodLibrary();
    const project = await persistCreateProject(gpxText, filename, migratedLibrary ?? undefined);
    setActiveProject(project);
    activeProjectRef.current = project;
    await refreshList();
    return project;
